/**
 * Aplicador de migraciones de MS-Contratos.
 *
 ...
 *
 * 1. Los `.sql` viven en `database/contratos/`, en la raiz del monorepo, que es
 *    donde CLAUDE.md ubica las migraciones: una carpeta por esquema. Salieron
 *    de `database/dominio/`, que despues de este paso solo guarda ya lo de
 *    Financiero y se vacia del todo en el 6e.
 *
 * 2. La tabla de control tambien vive en el esquema `contratos`, para que este
...
/** `<raiz del monorepo>/database/contratos`, desde `src/database/`. */
export const RUTA_BASE = path.resolve(__dirname, '../../../../database/contratos');
 ...
 *
 * Solo funciona con `NODE_ENV=test`, y solo borra `contratos`: aunque alguien lo
 * ejecutara por error contra la base de desarrollo, el esquema `public` del
 * gateway quedaria intacto.
 */
