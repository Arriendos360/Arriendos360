 ...
 * `allowNull`), la modificación en `beforeUpdate` y la actualización masiva en
 * `beforeBulkUpdate`. No muevas la modificación a `beforeValidate`:
 * `instancia.update()` descartaría el cambio de `actualizado_por`.
 */
export const registrarHooksAuditoria = (modelo: ModelStatic<Model>): void => {
  ...
  modelo.addHook('beforeBulkUpdate', (opciones) => {
    ...
    // `fields` ya viene calculado: hay que añadir la columna o Sequelize la ignora.
